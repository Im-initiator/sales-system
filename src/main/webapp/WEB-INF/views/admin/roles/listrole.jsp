...
<body>
<c:url var="urlRole" value="/api/admin/role"/>
<c:url var="urlList" value="/admin/role"/>
				  ...
				  <div class="modal-dialog">
				    <div class="modal-content">
				      ...
				      <div class="modal-body">
				        	<form id="formAdd">
							  <div class="mb-3">
							    <label for="categoryName" class="form-label">Authorization name</label>
				...
				<form action="<c:url value='/admin/category'/>" id="formSubmit" method="GET"> 
					<div class="table-responsive">    							       
							<table id="mytable" class="table table-bordred table-striped">         							
		    					...
		    					<tbody>
									<c:forEach items="${model}" var ="role" varStatus="loop">
									    <tr>									    	
									    	<td><input type="checkbox" value="${role.id}" id = "${role.id}"/></td>
									    	<input type="hidden" class="category${loop.index}" value="${role.id}"/>
										    <td class="category${loop.index}">${role.name}</td>
										    <td class="category${loop.index}">${role.code}</td>
		...
		$('#update').click(function(e){
			...
			function updateCategory(data){
				$.ajax({
					...
					success: function(result){
						$('#errorSystem').removeClass().addClass("alert alert-success");
						$('#errorSystem').text("Update thành công");
						$('#errorSystem').show();
						 setTimeout(function() {
						    	element.hide(); // Ẩn thông báo sau 3 giây
						    	element.removeClass("alert alert-success");
